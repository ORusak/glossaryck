...
turn - сервер когда адресат закрыт и прямого доступа к нему нет. За firewall устанавливается turn сервер к которому идет соединение и через который идут media потоки.
